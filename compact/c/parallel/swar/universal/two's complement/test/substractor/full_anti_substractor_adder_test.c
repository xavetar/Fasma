...
int main() {
    ...
    for (uint64_t A = 0x00; ; ) {
        for (uint64_t B = 0x00; ; ) {
            ...
            uint32_t iterations = count_iterations(A, B_temp, &final_difference);

            // Ожидаемое сложение или вычитание в 8-битной арифметике
            uint64_t expected;
            if (add_mode == true) {
                expected = anti_adder(A, B) & MASK; // anti_substractor(A, (~anti_substractor(B, ONE)) & MASK) & MASK;
            } else {
                expected = anti_substractor(A, B) & MASK;
            }
            ...
            if (B == MASK) { break; }; B = anti_substractor(B, MASK); // B + 1
        }
        if (A == MASK) { break; }; A = anti_substractor(A, MASK); // A + 1
    }
