/// Compares two __m128i vectors containing two 64-bit signed integers each, determining if a > b.
/// Returns a __m128i vector where each 64-bit lane is set to 0xFFFFFFFFFFFFFFFF if a > b, or
/// 0x0000000000000000 otherwise. The most significant bit (MSB, bit 63) is the sign bit
/// (1 = negative, 0 = positive).
///
/// Example:
///   a = [0x0000000000000005, 0x8000000000000000] (lane 0: 5, lane 1: -2^63)
///   b = [0x0000000000000003, 0x0000000000000001] (lane 0: 3, lane 1: 1)
///   Result = [0xFFFFFFFFFFFFFFFF, 0x0000000000000000] (lane 0: 5 > 3, lane 1: -2^63 < 1)
pub unsafe fn _mm_cmpgt_epi64(a: __m128i, b: __m128i) -> __m128i {
    ...
    // The MSB (bit 63) indicates the sign (0 = positive, 1 = negative). The XOR operation
    // yields 1 for differing MSBs (0 XOR 1 or 1 XOR 0) and 0 for matching MSBs (0 XOR 0 or
    // 1 XOR 1). The mask 0x8000000000000000 isolates the MSB, setting diff_signs[MSB]=1 in
    // lanes where MSBs differ, and 0 where they match. This distinguishes lanes requiring
    // sign-based handling (Step 4) from those handled by subtraction (Step 3).
    // Example:
    //   a = 0x0000000000000005 (5, MSB=0), b = 0x8000000000000000 (-2^63, MSB=1)
    //   XOR = 0x8000000000000005, diff_signs = 0x8000000000000000
    ...
    // For signed comparison, if b < a, the subtraction b - a yields a negative result with
    // MSB=1, indicating a > b. The mask isolates the MSB of the result.
    // Example:
    //   a = 0x0000000000000005 (5), b = 0x0000000000000003 (3)
    //   b - a = 3 - 5 = -2 = 0xFFFFFFFFFFFFFFFE
    ...
    // Compute sync = ~(a[MSB]) & b[MSB] to address lanes where MSBs differ (diff_signs[MSB]=1,
    // from 0 XOR 1 or 1 XOR 0). For signed comparison, when a[MSB]=0 and b[MSB]=1, a is
    // positive and b is negative, so a > b (e.g., 5 > -3). This operation sets sync[MSB]=1
    // in such lanes. The ~a[MSB] term is 1 when a[MSB]=0, and b[MSB] is 1 when b[MSB]=1,
    // capturing the case 0 XOR 1. The other differing case (a[MSB]=1, b[MSB]=0, 1 XOR 0)
    // implies a < 0, b >= 0, so a < b, and sync[MSB]=0. Matching MSB cases (0 XOR 0, 1 XOR 1)
    // are handled in Step 3, as diff_signs[MSB]=0 excludes them from needing sync. This step
    // ensures the correct result for differing MSBs by adding the result where a’s sign
    // indicates a larger value.
    // Example:
    //   a = 0x0000000000000005 (5, MSB=0), b = 0x8000000000000000 (-2^63, MSB=1)
    //   a[MSB] = 0x0000000000000000, ~a[MSB] = 0xFFFFFFFFFFFFFFFF
    //   b[MSB] = 0x8000000000000000
    //   sync = 0xFFFFFFFFFFFFFFFF & 0x8000000000000000 = 0x8000000000000000
    //   For a = 0x8000000000000000 (-2^63, MSB=1), b = 0x0000000000000001 (1, MSB=0):
    ...
    // the sign-based results (differing MSBs, Step 4). If either indicates a > b, the lane’s
    // MSB is set to 1.
    // Example:
    //   r = 0x8000000000000000 (lane 0: 5 > 3, Step 3)
    //   sync = 0x8000000000000000 (lane 1: 5 > -2^63, Step 4)
